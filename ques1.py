...
# Part 3: Tokenization and Top 30 Tokens for each CSV file
# Load the tokenizer for BERT
tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased")

# Function to count token occurrences from individual texts
def count_tokens(text):
    tokens = tokenizer.tokenize(text)  # Removed clean_up_tokenization_spaces argument
    token_counts = Counter(tokens)
    return token_counts

# Iterate over each CSV file and tokenize its respective text column
for csv_file, text_column in csv_files.items():
    # Get text from the specific CSV file's column
    csv_text = consolidated_texts[csv_file]
